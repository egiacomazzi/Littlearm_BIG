...
def check_command(command):
    '''
        checks angles for each servo and each part of the command if it's in the correct range
        (I took the correct ranges from the bars in the LittleArm Big app)
    '''
    print(command)
...
def move_it(command):
    '''
        ...
        more or less copied from LittleArmBIG_GUI_VO_4.1.py move_it function
    '''
    ser.flushInput()
    ser.flushOutput()
    ser.write(command)
...
def specific_position():
    '''
        ...
        error handling besides no input happens in check_command()
    '''
    try:
...
def wave(positions):
    '''
        takes list of different commands (positions) and moves the arm to one after the other

        time.sleep function is important because otherwise the Arduino accomulates the data
        you send and the arm isn't able to move correctly
        (probably better possibilities possible like checking if Arduino is ready again and then continue)
    '''
    try:
...
def press_button(ans):
    '''
        presses button in specific area
...
def emotional_reaction(positions):
    '''
        uses positions to move in an emotional ways

        try and error with the positions you think appear happy/sad/angry
    '''
    for i in positions:
        check_command(i)
        time.sleep(4)
...
#         try and error with the positions you think appear happy
#     '''
...
#         try and error with the positions you think appear sad
#     '''
...
#         try and error with the positions you think appear angry
#     '''
#     positions = []
#     for i in positions:
#         move_it(i)
...
def main():
    '''
        starts programm with connecting the serial (Arduino/Arm)
    '''
